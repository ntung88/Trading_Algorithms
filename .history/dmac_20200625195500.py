...
def main():
    ...
    # split = np.array_split(frame, 3)
    # periods = optimize(split[0])
    # print('Periods: ' + str(periods[:2]))
    # print('Test Profit: ' + str(-1 * periods[2]))
    # print('Validation Profit: ' + str(-1 * run_analysis(periods[:2], split[1])))

    periods = optimize(frame)

    visualize(pd.concat(split[:2]), periods[0], periods[1])
