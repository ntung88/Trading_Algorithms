...
def optimize(data):
    ...
    short_seeds = range(5, 200, 5)
    long_seeds = range(20, 1000, 5)
    minimum = float('inf')
    best_short = 0
    best_long = 0
    for short_seed in short_seeds:
        for long_seed in long_seeds:
            res = minimize(run_analysis, [short_seed, long_seed], args=data, method='COBYLA', constraints=cons, options={'rhobeg': 2.0, 'disp': True})
            if res < minimum:
                best_short = short_seed
    return res
