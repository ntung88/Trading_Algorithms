...
def optimize(data):
    ...
    cons = {'type': 'ineq', 'fun': lambda x: x[1] - x[0] > 1,
    'type': 'ineq', }
    res = minimize(run_analysis, [200, 700], args=data, method='nelder-mead', options={'disp': True})
    return res

def run_analysis(periods, data):
    short_period = periods[0]
    long_period = periods[1]
    sma = data.rolling(int(short_period)).mean()
    lma = data.rolling(int(long_period)).mean()
