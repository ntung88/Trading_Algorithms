...
def optimize(data):
    ...
    res = minimize(run_analysis, [5, 30], args=data, method='COBYLA', constraints=cons, options={'rhobeg': 2.0, 'disp': True})
    return res
...
def run_analysis(periods, data):
    ...
    print(round(short_period), round(long_period))
    sma = data.rolling(round(short_period)).mean()
    lma = data.rolling(round(long_period)).mean()
...
def main():
    ...
    print(optimize(frame).x.astype(int))
