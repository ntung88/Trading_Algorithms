...
def optimize(data):
    ...
    res = minimize(run_analysis, [100, 300], args=(data), method='nelder-mead', options={'disp': True})
    return res

def run_analysis(periods, args):
    data = args[0]
    short_period = periods[0]
    long_period = periods[1]
    print(short_period.value, long_period.value)
    sma = data.rolling(short_period.value).mean()
    lma = data.rolling(long_period.value).mean()
